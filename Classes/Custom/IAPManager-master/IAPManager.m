...
{
    if (self.productRequests.count > 0)
    {
        ProductsCompletionBlock completion = [self.productRequests[0] objectAtIndex:1];
...
- (void)purchaseProductForId:(NSString *)productId completion:(PurchaseCompletionBlock)completionBlock error:(ErrorBlock)err {
    ...
    {
        if ([products isKindOfClass: [NSError class]])
        {
            err((NSError*)products);
            return ;
        }
        
        if([products count] == 0)
            
            err([NSError errorWithDomain:@"IAPManager" code:0 userInfo:[NSDictionary dictionaryWithObject:[NSString stringWithFormat:@"Didn't find products with ID %@", productId] forKey:NSLocalizedDescriptionKey]]);
        else
            [self purchaseProduct:[products objectAtIndex:0] completion:completionBlock error:err];
    }];
#endif
}
